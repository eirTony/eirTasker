...
void Enumeration::set(const BasicName & name)
{
...
BasicName Enumeration::name(void) const
{
    return name_s;
}
...
bool Enumeration::isValid(const BasicName & name)
{
    return (name_map && name_map->contains(name));
}

int Enumeration::value(const BasicName & name)
{
    int result = -1;
    if (name_map)
    {
